...
from .table_post_process import apply_threshold, nms, align_supercells, nms_supercells, header_supercell_tree, refine_rows, refine_columns, align_columns, align_rows, slot_into_containers
from .table_post_process import extract_text_from_spans, sort_objects_top_to_bottom, sort_objects_left_to_right
...
class TableExtractionPipeline(object):
    def __init__(self, det_device=None, str_device=None,
        ...
        self.det_model = AutoModelForObjectDetection.from_pretrained(det_model_path, revision="no_timm", local_files_only = True)
        self.det_model.to(det_device)
        self.det_model.eval()

        self.str_model = TableTransformerForObjectDetection.from_pretrained(str_model_path, local_files_only = True)
    ...
    def detect(self, img,):
        ...
        img_tensor = detection_transform(img).unsqueeze(0)
    ...
    def recognize(self, img, tokens=None):
        ...
        # Transform the image how the model expects it
        img_tensor = structure_transform(img).unsqueeze(0)
