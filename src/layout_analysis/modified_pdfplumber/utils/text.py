...
class WordExtractor:
    def __init__(
        ...
        expand_ligatures: bool = True,
        x_tolerance_rate = 0.1
    ):
        self.x_tolerance = x_tolerance
        self.y_tolerance = y_tolerance
        self.x_tolerance_rate = x_tolerance_rate
    ...
    def char_begins_new_word(
        ...
        *top* of the previous character to the *top* of the next
        ...
        if curr_char["upright"]:
            ...
            intra_tol = self.x_tolerance_rate * max(curr_char['x1'] - curr_char['x0'], curr_char['bottom'] - curr_char['top'], prev_char['x1'] - prev_char['x0'], prev_char['bottom'] - prev_char['top'])
            inter_attr = "top"
            intra_attr_min = "x0"
            intra_attr_max = "x1"
        ...
        return bool(
            ...
            # Interline test
            or (abs(char_max[inter_attr] - char_min[inter_attr]) > inter_tol)   # 这里有新的改动
        )
    ...
    def  iter_extract_tuples(
        ...
        # grouping_key = itemgetter("upright", *self.extra_attrs)  这里有修改
        grouping_key = itemgetter("upright", "size")
