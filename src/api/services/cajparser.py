...
from api.services.utils import fnd, fnd_all, add_outlines, fnd_rvrs, fnd_unuse_no, find_redundant_images
try:
...
class CAJParser(object):
 ...
 #           self._convert_hn(dest)
    ...
    def _convert_kdh(self, dest):
        ...
        doc = pymupdf.open(dest + '.tmp')
        doc.save(dest,clean=True)

        print('1')

        # Use mutool to repair xref
        # try:
        #     os.system(r"D:\Edge_download\other_file\mupdf-1.18.0-windows\mutool.exe clean {} {}".format(dest+".tmp", dest))
        #     # print(0)
        #     # ret = check_output(["mutool", "clean", dest + ".tmp", dest], stderr=STDOUT)
        #     # print(ret)
        #     # print(1)
        # except CalledProcessError as e:
        #     print(e.output.decode("utf-8"))
