...
from document_fragment.ooxml_to_latex.symbols import (CHARS, CHR, CHR_BO, CHR_DEFAULT, POS, POS_DEFAULT
...
def load(stream):
    ...
    for o_math in tree.findall(OMML_NS + 'oMath'):
        yield oMath2Latex(o_math)


def load_string(string):
    root = ET.fromstring(string.encode('utf-8'))
    for o_math in root.findall(OMML_NS + 'oMath'):
        yield oMath2Latex(o_math)
